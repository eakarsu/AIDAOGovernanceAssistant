/**
 * Custom Feature: voter-education
 * Proposal pro/con summaries
 *
 * POST /api/ai/voter-education
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_VOTER_EDUCATION_KEY
...
const FEATURE_KEY = process.env.FEATURE_VOTER_EDUCATION_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Proposal pro/con summaries.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDAOGovernanceAssistant - voter-education',
    },
    body: JSON.stringify({
...
router.post('/voter-education', auth, async (req, res) => {
  try {
    ...
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_VOTER_EDUCATION_KEY');
    const ai = await callLLM({ feature: 'voter-education', goal: 'Proposal pro/con summaries', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['voter-education', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[voter-education] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'voter-education',
      endpoint: '/api/ai/voter-education',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[voter-education] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/voter-education/health', (req, res) => {
  res.json({
    feature: 'voter-education',
    endpoint: '/api/ai/voter-education',
