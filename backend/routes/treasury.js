...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const [countResult, dataResult] = await Promise.all([
      pool.query('SELECT COUNT(*) FROM treasury_transactions'),
      pool.query('SELECT * FROM treasury_transactions ORDER BY transaction_date DESC LIMIT $1 OFFSET $2', [limit, offset])
    ]);
